...
const MAX_PARSEABLE_FILE_SIZE = 12 * 1024 * 1024;
...
function normalizeExtractedText(
  ...
  if (rawBytes <= byteLimit) {
    const excerpt = compact
      ? compact.replace(/\s+/g, " ").trim().slice(0, 800)
      : null;
    return {
      text: compact,
      excerpt,
  ...
  const truncatedText = Buffer.from(compact, "utf8")
...
async function extractWithCliTool(
  ...
  | { ok: true; text: string }
  | { ok: false; missingTool: boolean; error: string }
> {
  try {
...
export async function readAttachmentContent(
  ...
  const mimeType = String(mimeTypeRaw || "")
    .trim()
    .toLowerCase();
  const textByteLimit = normalizeLimit(
    options.textByteLimit,
    DEFAULT_TEXT_BYTE_LIMIT,
    800_000,
  );
  const base64ByteLimit = normalizeLimit(
    options.base64ByteLimit,
    DEFAULT_BASE64_BYTE_LIMIT,
    512_000,
  );
  ...
  const parsedFilename = path.basename(filePath);
  const fallbackUrl = `/api/files/${encodeURIComponent(parsedFilename)}`;

  if (fileSize > MAX_PARSEABLE_FILE_SIZE) {
    const sizeMb = (fileSize / (1024 * 1024)).toFixed(1);
    const limitMb = MAX_PARSEABLE_FILE_SIZE / (1024 * 1024);
    return {
      status: "too_large",
      parser: "none",
      note: `File is ${sizeMb}MB, exceeds the ${limitMb}MB parsing limit.`,
      mimeType,
      supportedMimeTypes: Array.from(ALL_SUPPORTED_MIME_TYPES),
      text: null,
      excerpt: null,
      ...
      base64Truncated: false,
      fileSize,
      parseError: `File too large for server-side parsing (${sizeMb}MB). Download directly instead.`,
      fallbackUrl,
    };
  }
