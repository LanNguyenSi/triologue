...
- `describeRunError`/`RunError` moved out of `PluginWorkspacePage.tsx` into `client/src/lib/runError.ts` (with its own direct unit tests), since it was the only non-component export under `pages/`; `PluginWorkspacePage` now imports it instead of defining it locally. Known remaining follow-up (not done here): `FilesPage.tsx` independently carries the same `RunError`-shaped union and seven inline copies of this shaping expression (`error instanceof Error ? { message: error.message } : ...`, including the same empty-message bug this helper fixes) and has not been converted to use the shared helper.
