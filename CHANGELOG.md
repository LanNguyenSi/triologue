...
- `DELETE /api/auth/me` (self-deletion) no longer 500s for any user who ever caused an `agent_audit_log` row to be written (for example, PATCHing a task once: `routes/projects.ts`'s `updateTask` fires an unconditional, un-awaited `logAuditEvent` on every successful call). `AgentAuditLog.agentId` had no `onDelete` rule, so `prisma.user.delete()` hit Postgres's default blocking foreign-key action and the route's bare `catch {}` turned that into an opaque `{"error":"Failed to delete account."}` with the user row still present. Decision: anonymise, not delete or cascade-delete, a departing user's audit trail. `AgentAuditLog.agentId` is now nullable with `onDelete: SetNull` (migration `20260923045824_agent_audit_log_agentid_nullable_setnull`, hand-scoped from Prisma's own generated diff to this one relation after `prisma migrate dev --create-only` also surfaced unrelated, pre-existing drift between migration history and `schema.prisma` on four other tables that this task does not touch). That FK rule alone only nulls `agentId`; the same `prisma.$transaction` as the user delete now also scrubs `details` (see `docs/okf/prisma-data-model-invariants.md`, Invariant 6, for the full enumeration): JSON `null` on every row this user wrote (a task title, a screening run's title, an approval's decision note, an attachment's filename can otherwise be the only surviving copy once its source row is cascade-deleted with them), and the `assignedTo` key removed, via the jsonb `-` operator, from any other still-present user's row that names this user's id there. The route's `catch` block also distinguishes a known Prisma constraint failure (`Prisma.PrismaClientKnownRequestError` with `code === 'P2003'`, logged with the code and Prisma's `meta`, returned as `409` with the code in the body) from any other unexpected error (still logged and `500`), instead of collapsing every failure into the same generic message. `routes/projects.ts`'s `GET /:projectId/activity` (one of two other readers of `AgentAuditLog.agentId` outside tests, besides the write-only `services/auditService.ts`) no longer builds its agent lookup from a `String(item.agentId)` that would coerce a null agent into the literal string `"null"`; it now filters out rows whose acting user has been deleted before querying, so those rows render with an absent `agentName`/`agentUsername` instead of matching a stray `"null"` id (`client/src/services/auditApi.ts`'s `AuditEntry.agentId` is now typed `string | null` to match, and `ProjectActivityPage` falls back to a translated "deleted user" label instead of rendering a blank row). The other reader, `routes/agents.ts`'s `GET /api/agents/audit`, always filters on the calling agent's own current id and never selects `agentId` back out, so it needed no change. DB-backed tests in `server/src/__tests__/auth-self-delete.test.ts` (gated on `RUN_DB_TESTS=1` like the repo's other integration suites) reproduce the exact tracker repro (register, create a project and task, PATCH the task once, `DELETE /me`) and assert `200` plus the surviving audit row's `agentId` being `null`; pin the `409`-vs-`500` catch-arm classification, including the logged Prisma error code, by mocking `prisma.$transaction` to reject with each error shape; pin both `details` scrub statements across a two-actor task-reassignment scenario; and pin `GET /:projectId/activity`'s anonymised-row rendering after a project member self-deletes.
