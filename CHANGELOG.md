...
- Follow-up to the multer 2.3.0 bump above: `file.originalname` now decoded raw CR/LF/`"` before it reached logging or persistence. A new `stripControlChars` helper (`server/src/utils/sanitizeFilename.ts`) strips ASCII control characters (code points 0-31 and 127, not the double quote) from `originalname` at every site that logs or persists it or derives an on-disk extension from it: `server/src/routes/upload.ts`, `server/src/routes/projects.ts` (both project- and task-attachment paths) and `server/src/plugins/builtin/salesWorkbenchPlugin.ts`. `server/src/routes/userFiles.ts`'s SharePoint target-name path is unchanged: it already re-encodes the path before use. `server/src/__tests__/upload.test.ts` also gains the `limits.fileSize` exact-boundary case the same bump moved (a file of exactly `MAX_FILE_SIZE` bytes is now accepted, beside the existing "exceeding 10 MB -> 413" case).
