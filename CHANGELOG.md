...
- `DELETE /api/auth/me` (self-deletion) no longer 500s for any user who ever caused an `agent_audit_log` row to be written (for example, PATCHing a task once: `routes/projects.ts`'s `updateTask` fires an unconditional, un-awaited `logAuditEvent` on every successful call). `AgentAuditLog.agentId` had no `onDelete` rule, so `prisma.user.delete()` hit Postgres's default blocking foreign-key action and the route's bare `catch {}` turned that into an opaque `{"error":"Failed to delete account."}` with the user row still present. Decision: anonymise, not delete or cascade-delete, a departing user's audit trail. `AgentAuditLog.agentId` is now nullable with `onDelete: SetNull` (migration `20260923045824_agent_audit_log_agentid_nullable_setnull`, hand-scoped from Prisma's own generated diff to this one relation after `prisma migrate dev --create-only` also surfaced unrelated, pre-existing drift between migration history and `schema.prisma` on four other tables that this task does not touch). Every other `agent_audit_log` column was checked for the deleted user's personal data (see `docs/okf/prisma-data-model-invariants.md`, Invariant 6, for the full enumeration and reasoning): none carry it, so only `agentId` needed the anonymising rule. The route's `catch` block now also distinguishes a known Prisma constraint failure (`Prisma.PrismaClientKnownRequestError` with `code === 'P2003'`, logged with the code and Prisma's `meta`, returned as `409` with the code in the body) from any other unexpected error (still logged and `500`), instead of collapsing every failure into the same generic message. `routes/projects.ts`'s `GET /:projectId/activity` (the one other reader of `AgentAuditLog.agentId` outside tests, besides the write-only `services/auditService.ts`) no longer builds its agent lookup from a `String(item.agentId)` that would coerce a null agent into the literal string `"null"`; it now filters out rows whose acting user has been deleted before querying, so those rows render with an absent `agentName`/`agentUsername` instead of matching a stray `"null"` id. New DB-backed test `server/src/__tests__/auth-self-delete.test.ts` (gated on `RUN_DB_TESTS=1` like the repo's other integration suites) reproduces the exact tracker repro (register, create a project and task, PATCH the task once, `DELETE /me`) and asserts `200` plus the surviving audit row's `agentId` being `null`, and separately pins the `409`-vs-`500` catch-arm classification by mocking `prisma.user.delete` to reject with each error shape.
