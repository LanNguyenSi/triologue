...
`POST /api/agents` (agents.ts:546, docblock 533-545): any authenticated user
may create an agent. Flow:

1. `mentionKey = toMentionKey(name)` — lowercase, strip everything outside
   `[a-z0-9_]` (agents.ts:100-102). Agent `User.username` is
   `agent_<mentionKey>_<4-byte-hex>` (agents.ts:564-565), so agent usernames
   never collide.
2. mentionKey uniqueness is checked **only against `AgentToken.mentionKey`**
   (agents.ts:570-582, 409 `AGENT_MENTION_KEY_TAKEN`).
3. Tiered activation (agents.ts:583-594): if the creator has
   `canTriggerAI === true` (agents.ts:586), the agent is auto-activated —
   `status: "active"`, `isActive: true` on both `AgentToken` and its `User`
   (agents.ts:588, 603, 615-616) — and `trustLevel` is **capped to
   `"standard"`**; elevated always requires an admin (agents.ts:590-594).
...
4. The agent's `User` is created with `canTriggerAI: false` — "Agents must not
   trigger other agents — prevents loops" (agents.ts:604).
5. Atomic transaction adds the agent to the hidden `"registration"` staging
   room, plus optionally one more room (agents.ts:597-656). `delivery` defaults
   to `"sse"` at this route (accepted set `["sse","webhook","openclaw-inject"]`,
   agents.ts:625-627); `receiveMode` defaults `"mentions"` (agents.ts:622-624).

Admin activation/rejection: `PATCH /api/agents/:id/activate` (agents.ts:1152)
sets `status`/`isActive` and mirrors `isActive` onto the agent's `User` record
in one transaction. Soft-delete: `DELETE /api/agents/:id` (agents.ts:1197),
creator or admin only. The gateway bootstraps its agent roster from
`GET /api/agents/gateway-config` (agents.ts:477-531) — gateway-token-gated
(agent username must be `gateway` or `gateway-agent-001`, agents.ts:492-499),
...
422, agents.ts:2162, 2285-2294), room participation (2316-2324), and create
...
**2. Per-agent send limits** (agents.ts:2252-2314, in-memory, resets on
restart): sliding 60s window (`AGENT_RATE_LIMIT_WINDOW_MS`, agents.ts:2165)
capped at `config.maxMessagesPerMinute` (default 5, agents.ts:2252) → 429 with
`retryAfterMs`; plus near-duplicate suppression per agent+room — Jaccard
similarity ≥ 0.8 within 5s → 429 (`DEDUP_WINDOW_MS`,
`DEDUP_SIMILARITY_THRESHOLD`, agents.ts:2163-2164, 2296-2310).
