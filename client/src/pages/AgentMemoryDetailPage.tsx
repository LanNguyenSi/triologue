...
export const AgentMemoryDetailPage: React.FC = () => {
  ...
  return (
    <PageShell
      ...
      actions={
        <>
          <Button type="button" variant="secondary" onClick={() => navigate("/memory")}>
            {t("memory.detail.back")}
          </Button>
          {entry?.editable && !entry.archivedAt && (
            <Button type="button" onClick={() => navigate(`/memory/${entry.id}/edit`)}>
              {t("memory.list.edit")}
            </Button>
          )}
        </>
              ...
              <div className="flex flex-wrap gap-2">
                {entry.editable && !entry.archivedAt && (
                  <Button
                    ...
                    variant="danger"
                    onClick={() => {
                      if (window.confirm(t("memory.detail.archiveConfirm"))) {
                        void archiveEntry();
                      }
                    }}
                    disabled={saving}
