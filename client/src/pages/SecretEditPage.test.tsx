...
function mountSecretEdit(apiClientMock: ReturnType<typeof vi.fn>) {
  ...
  ]).then(([{ SecretEditPage }, languageContextModule]) => {
...
describe("SecretEditPage does not refetch the secret on a real language switch (AC1, a34078b6)", () => {
  it("keeps loadSecret's fetch at 1 call after a real setLanguage, loadProjects unaffected", async () => {
    ...
    await screen.findByDisplayValue("MY_SECRET");
    await waitFor(() => expect(secretCalls()).toBe(1));
    expect(
      screen.getByText("Namen, Wert und Zuordnung aktualisieren."),
    ).toBeTruthy();
    ...
    await screen.findByText("Update name, value, and project assignment.");
    await waitFor(() => expect(secretCalls()).toBe(1));
  });
});
